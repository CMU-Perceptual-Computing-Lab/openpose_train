...
## Path parameters
import os
sCaffeFolder =  '/home/yuand/codes/openpose_caffe_train/'
sDatasetFolder = '/media/posefs0c/Users/yuand/'
sLmdbFolders = ['lmdb_dome_fixed/']
sMediaDirectoryDome = '/media/posefs0c/panoptic/'
sPretrainedModelPath = '/media/posefs0c/Users/yuand/openpose_train/training_results/pose/openpose_pretrain_depth.caffemodel'
sTrainingFolder = '../../training_results/pose_fixed/'
...
sNumberKeyPoints = 18; # 18, Gines: 22
sNumberPAFs = 2*19; # 2*19, Gines: 2*23
sNumberDepth = 1
sNumberTotalParts = sNumberKeyPoints + sNumberPAFs
sNumberKeyPointsPlusBackground = sNumberKeyPoints+1;
sImageScale = 256;
sLearningRateInit = 1e-5   # 4e-5, 2e-5
sBatchSizes = [8] # [10], Gines: 21
sMaxRotationDegree = 40 # 40, Gines: 180
sBatchNorm = 0
sNumberStages = 6
sScaleMin = 0.9 # 0.5, Gines: 0.1
sScaleMax = 1.1
...
sKeypointLevel = 1
sPafLevel = 0
sLabelName = ['label_vec', 'label_heat', 'label_depth', 'vec_weight', 'heat_weight', 'depth_weight', 'vec_temp', 'heat_temp', 'depth_temp'] # `heat` in [3*n+sKeypointLevel] positions, `vec` in [3*n+sPafLevel] positions
...
def setLayersTwoBranches(dataFolders, batchSizes, layerName, kernel, stride, numberOutputChannels, numberTotalParts, labelName,
    ...
    # Training mode - Use lmdb
        ...
        # If merging different datasets
            ...
            if len(batchSizes) == 2:
                ...
                caffeNet.image = L.Concat(caffeNet.tops['dataCoco'], caffeNet.tops['dataDomeDB'], concat_param=dict(axis=0))
                caffeNet.tops['label'] = L.Concat(caffeNet.tops['labelCoco'], caffeNet.tops['labelDomeDB'], concat_param=dict(axis=0))
        else:
            raise ValueError("Wrong batchSizes size.")
        # Slice layer
        caffeNet.tops[labelName[3]], caffeNet.tops[labelName[4]], caffeNet.tops[labelName[6]], caffeNet.tops[labelName[7]], caffeNet.tops[labelName[5]], caffeNet.tops[labelName[8]] = L.Slice(
            caffeNet.label, slice_param=dict(axis=1, slice_point=[sNumberPAFs, numberTotalParts+1, numberTotalParts+sNumberPAFs+1, 2*numberTotalParts+2, 2*numberTotalParts+3]), ntop=6
        )
        # Eltwise layers
        for level in range(0,3):
            caffeNet.tops[labelName[level]] = L.Eltwise(caffeNet.tops[labelName[level+3]], caffeNet.tops[labelName[level+6]], operation=P.Eltwise.PROD)

    # just follow arrays..CPCPCPCPCCCC....
    lastLayer = ['image', 'image', 'image']
    stage = 1
    convCounter = 1
    poolCounter = 1
    dropCounter = 1
    ...
    for l in range(0, len(layerName)):
        ...
        # Convolution layers
            ...
            for level in range(0,3):
                ...
                if layerName[l+1] == 'L2' or layerName[l+1] == 'L3':
                    ...
                    elif level == sDepthLevel:
                        numberOutputChannels[l] = sNumberDepth
                    else:
        ...
        # Loss layers
            ...
            if not deploy:
                ...
                caffeNet.tops[name] = L.Eltwise(caffeNet.tops[lastLayer[level]], caffeNet.tops[labelName[(level+3)]], operation=P.Eltwise.PROD)
        ...
        # Concat layers
            ...
            caffeNet.tops['concat_stage%d' % stage] = L.Concat(caffeNet.tops[lastLayer[0]], caffeNet.tops[lastLayer[1]], caffeNet.tops[lastLayer[2]], caffeNet.tops[sharePoint], concat_param=dict(axis=1))

            localCounter = 1
            state = 'fuse'
            lastLayer[0] = 'concat_stage%d' % stage
            lastLayer[1] = 'concat_stage%d' % stage
            lastLayer[2] = 'concat_stage%d' % stage
            print lastLayer
    ...
    # Return result
    if deploy:
        caffeNet.tops['net_output'] = L.Concat(caffeNet.tops[lastLayer[0]], caffeNet.tops[lastLayer[1]], caffeNet.tops[lastLayer[2]], concat_param=dict(axis=1))
        deployInit = 'input: {}\n\
...
def getBash():
    return ('#!/usr/bin/env sh\n' + sCaffeFolder + '/build/tools/caffe train --solver=$2 --gpu=$1 --weights=${3-'+sPretrainedModelPath+'} 2>&1 | tee ./training_log.txt')



if __name__ == "__main__":
    sLearningRateMultDistro = [1.0, 1.0, 4.0, 1.0]
    transformParamCoco = dict(stride=8, crop_size_x=sImageScale, crop_size_y=sImageScale,
                              ...
                              visualize=False, model='COCO')
    transformParamDome = eval(repr(transformParamCoco))
    transformParamDome['model'] = 'DOME'
    transformParamDome['media_directory'] = sMediaDirectoryDome
    #transformParams = [transformParamCoco, transformParamDome]
    transformParams = [transformParamDome]

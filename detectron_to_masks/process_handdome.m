% Prepares hand-dome data in coco format with 21 keypoints.
...
hand_dome_dataset = [main_folder, 'hand143_panopticdb/'];
path_im = [hand_dome_dataset, 'imgs/'];
path_im_val = [hand_dome_dataset, 'imgs_val/'];
path_ann = [main_folder, 'json/hand21_dome_train.json'];
path_dec = [hand_dome_dataset, 'imgs_det_out/'];
tempMatFile = [hand_dome_dataset, 'handdome_gt_clean.mat'];

file_in = path_ann;

% train_out = './processed_handdome_train.json';
% val_out = './processed_handdome_val.json';
train_out = [main_folder, 'hand21_dome_train.json'];
val_out = [main_folder, 'hand21_dome_val.json'];
...
keypoints = keypoints(22:end); % Dome only label right hand
skeleton = skeleton-1; % json is 0-indexed

dj_out = []; % data_json_out
dj_out.info = dj.info;
dj_out.info.version = '1.1 [Created by Haroon Idrees]';
dj_out.info.description = ['CMU Panoptic Studio Hands dataset.'...
...
unlabeled_indices = []; 
% all 21 labeled; set keypoint status to 2; 
% should be 42; set keypoint status of rest to 3; 
...
%             ind_kp_valid = find(dj_out.annotations(end).keypoints([1:42]*3) == 2);
            ind_kp_valid = find(dj_out.annotations(end).keypoints([1:21]*3) == 2);
...
%             dj_out.annotations(end).keypoints([1:42]*3) = 3;
            dj_out.annotations(end).keypoints([1:21]*3) = 3;
